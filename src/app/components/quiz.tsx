...
// Liest alle Texte, Bilder und Farben aus dem CMS (component_quiz)
// Fallbacks = aktuelle Hardcode-Werte → sieht ohne DB-Einträge identisch aus
...
// ── CMS Defaults (exakt die aktuellen Hardcode-Werte) ─────────────────────────
const D = {
  brand:         '#884A4A',
  quiz_bg:       '#f7f7f7',
  // Header
  ...
  q1_opt1:       'Ja, ich bin gespannt',
  q1_opt2:       'Noch unsicher',
  q1_img1:       '/option11.jpg',
  q1_img2:       '/option22.jpg',
  // Q2
  q2_question:   'Hast du bereits Erfahrung im Singen?',
  q2_opt1:       'Ja, im Kirchenchor',
  q2_opt2:       'Ja, in der Band',
  q2_opt3:       'Sonstige Erfahrung',
  q2_opt4:       'Noch keine Erfahrung',
  q2_img1:       '/kirchenchor.jpg',
  q2_img2:       '/band.jpg',
  q2_img3:       '/sonstige-erfahrung.jpg',
  q2_img4:       '/keine-erfahrung.jpg',
  // Q3
  q3_question:   'Interessiert du dich für vor-Ort Unterricht oder online?',
  q3_opt1:       'Vor-Ort Unterricht',
  q3_opt2:       'Online-Unterricht',
  // Form
  ...
  return (
    <button type="button" onClick={onClick}
      ...
      <div className="relative aspect-square w-full overflow-hidden">
        ...
        <div className={`absolute inset-x-0 bottom-0 text-center ${compact ? 'px-2 py-2.5 md:px-2 md:py-3' : 'px-4 py-4'}`}
          style={{ backgroundColor: `${brand}E6` }}>
          <span className={`block font-bold text-white ${compact ? 'text-xs leading-snug md:text-sm' : 'text-sm md:text-base'}`}>{title}</span>
        </div>
      </div>
    </button>
  );
...
function IconAnswerCard({ title, icon: Icon, onClick, brand }: { title: string; icon: React.ComponentType<{ className?: string }>; onClick: () => void; brand: string }) {
  return (
    <button type="button" onClick={onClick}
      ...
      <span className="px-4 text-base font-bold leading-snug md:text-lg">{title}</span>
    </button>
  );
}
...
export default function Quiz() {
  ...
  // Load CMS content
  useEffect(() => {
    fetch('/api/cms/public?page=component_quiz')
      .then(r => r.json())
      .then(json => {
        ...
        setCfg({
          brand:         v('colors::brand',              D.brand),
          quiz_bg:       v('colors::quiz_bg',            D.quiz_bg),
          header_title:  v('quiz_section::title',        D.header_title),
          header_sub:    v('quiz_section::subtitle',     D.header_sub),
          q1_question:   v('quiz_q1::question',          D.q1_question),
          q1_subtitle:   v('quiz_q1::subtitle',          D.q1_subtitle),
          q1_opt1:       v('quiz_q1::option_1',          D.q1_opt1),
          q1_opt2:       v('quiz_q1::option_2',          D.q1_opt2),
          q1_img1:       v('quiz_q1::img_1',             D.q1_img1),
          q1_img2:       v('quiz_q1::img_2',             D.q1_img2),
          q2_question:   v('quiz_q2::question',          D.q2_question),
          q2_opt1:       v('quiz_q2::option_1',          D.q2_opt1),
          q2_opt2:       v('quiz_q2::option_2',          D.q2_opt2),
          q2_opt3:       v('quiz_q2::option_3',          D.q2_opt3),
          q2_opt4:       v('quiz_q2::option_4',          D.q2_opt4),
          q2_img1:       v('quiz_q2::img_1',             D.q2_img1),
          q2_img2:       v('quiz_q2::img_2',             D.q2_img2),
          q2_img3:       v('quiz_q2::img_3',             D.q2_img3),
          q2_img4:       v('quiz_q2::img_4',             D.q2_img4),
          q3_question:   v('quiz_q3::question',          D.q3_question),
          q3_opt1:       v('quiz_q3::option_1',          D.q3_opt1),
          q3_opt2:       v('quiz_q3::option_2',          D.q3_opt2),
          form_title:    v('quiz_form::title',           D.form_title),
          form_subtitle: v('quiz_form::subtitle',        D.form_subtitle),
          form_name:     v('quiz_form::name_label',      D.form_name),
          form_email:    v('quiz_form::email_label',     D.form_email),
          form_phone:    v('quiz_form::phone_label',     D.form_phone),
          form_submit:   v('quiz_form::submit_label',    D.form_submit),
  ...
  const startLoadingStep = (data: Partial<Answers>) => {
    ...
    setLoading(true);
    setStep(3);
    if (!trackedSteps.current.has('q3')) { trackedSteps.current.add('q3'); trackFunnelStep('q3'); }
    if (loadingTimeoutRef.current) clearTimeout(loadingTimeoutRef.current);
    loadingTimeoutRef.current = setTimeout(() => {
      setLoading(false);
      setStep(4);
      if (!trackedSteps.current.has('form')) { trackedSteps.current.add('form'); trackFunnelStep('form'); }
  ...
  const next = (data: Partial<Answers>) => {
    ...
    if (stepKey && !trackedSteps.current.has(stepKey)) { trackedSteps.current.add(stepKey); trackFunnelStep(stepKey); }
  ...
  return (
    <div className="mx-auto w-full max-w-5xl">
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Open+Sans:wght@400;600;700;800&display=swap'); .quiz-open-sans { font-family: 'Open Sans', sans-serif; }`}</style>
        ...
        <div className="mt-6 md:mt-8">
          ...
          {step === 0 && (
            <section className="text-center">
              <h2 className="text-3xl font-extrabold md:text-4xl" style={{ color: graphite }}>{cfg.q1_question}</h2>
              <p className="mx-auto mt-3 max-w-lg text-base" style={{ color: lightGray }}>{cfg.q1_subtitle}</p>
              <div className="mx-auto mt-8 grid max-w-[520px] grid-cols-2 gap-3 md:gap-4">
                {[
                  { opt: cfg.q1_opt1, img: cfg.q1_img1 },
                  { opt: cfg.q1_opt2, img: cfg.q1_img2 },
                ].map(({ opt, img }) => (
          ...
          {step === 1 && (
            <section className="text-center">
              <h2 className="text-3xl font-extrabold md:text-4xl" style={{ color: graphite }}>{cfg.q2_question}</h2>
              <div className="mt-8 grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
                {[
                  { opt: cfg.q2_opt1, img: cfg.q2_img1 },
                  { opt: cfg.q2_opt2, img: cfg.q2_img2 },
                  { opt: cfg.q2_opt3, img: cfg.q2_img3 },
                  { opt: cfg.q2_opt4, img: cfg.q2_img4 },
                ].map(({ opt, img }) => (
                  <ImageAnswerCard key={opt} title={opt} image={img} compact brand={brand} onClick={() => next({ experience: opt })} />
                ))}
              </div>
              <div className="mt-6 text-center">
                <button type="button" onClick={back} className="text-sm underline underline-offset-4" style={{ color: lightGray }}>Zurück</button>
              </div>
            </section>
          )}
          ...
          {step === 2 && (
            <section className="text-center">
              <h2 className="text-3xl font-extrabold md:text-4xl" style={{ color: graphite }}>{cfg.q3_question}</h2>
              <div className="mx-auto mt-8 grid max-w-[520px] grid-cols-2 gap-3 md:gap-4">
                {[
                  { opt: cfg.q3_opt1, icon: MapPin },
                  { opt: cfg.q3_opt2, icon: Monitor },
                ].map(({ opt, icon }) => (
          ...
          {step === 4 && (
            <section className="mx-auto max-w-2xl text-center">
              ...
              <h2 className="mt-3 text-3xl font-extrabold md:text-4xl" style={{ color: graphite }}>{cfg.form_title}</h2>
              ...
              <form onSubmit={submitLead} className="mx-auto mt-8 max-w-xl space-y-3">
                ...
                <input className="h-12 w-full rounded-[4px] border border-neutral-300 bg-white px-4 outline-none transition focus:border-[color:var(--brand)]"
                  style={{ color: graphite, '--brand': brand } as React.CSSProperties}
                  name="name"  placeholder={cfg.form_name}  required />
                <input className="h-12 w-full rounded-[4px] border border-neutral-300 bg-white px-4 outline-none transition focus:border-[color:var(--brand)]"
                  style={{ color: graphite, '--brand': brand } as React.CSSProperties}
                  type="email" name="email" placeholder={cfg.form_email} required />
                <input className="h-12 w-full rounded-[4px] border border-neutral-300 bg-white px-4 outline-none transition focus:border-[color:var(--brand)]"
                  style={{ color: graphite, '--brand': brand } as React.CSSProperties}
                  type="tel"   name="phone" placeholder={cfg.form_phone} required pattern="^[0-9+()\s-]{6,}$" />
